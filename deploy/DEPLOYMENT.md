...
The app listens on the port set by `PORT` in `.env` (3003 by default). Make
sure nothing already holds it — if you ran `npm run dev` while setting up, it
is still listening and the service will fail to start with `EADDRINUSE`:
...
Ports 3003 and 3001 must **not** appear. Both are bound to loopback already;
...
| `EADDRINUSE: address already in use 127.0.0.1:3003` | Something else holds the port — usually an `npm run dev` left over from setup. `sudo ss -ltnp \| grep :3003`, stop it, then `sudo systemctl restart cafe-lsaf`. |
| 502 right after changing the port | `PORT` in `.env` and `proxy_pass` in `/etc/nginx/snippets/cafe-proxy.conf` disagree. They must match; nginx cannot read `.env`. |
